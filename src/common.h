...
char *format_time(const char *frm)
{
	static char buff[300];
	strftime(buff,100,frm,global.timeinfo);
	return buff;
}
char *ParsString(const char *str)
{
...
void RecordDemo()
{
	...
	try
	{
		std::string c_map(ParsString(global.Mapname.c_str()));
		std::string c_red(ParsString(REDTEAM));
		std::string c_blu(ParsString(BLUTEAM));
		std::string c_tag(ParsString(PREC_tag.GetString()));
 		RecCom=ParsString(format_time(Settings("DemoTime_format")));
		file=ParsString(Settings("DemoName_format"));
		pos=file.find("%date%");
		if(pos!= std::string::npos) file.replace(pos,6,RecCom);
		pos=file.find("%map%");
		if(pos!= std::string::npos) file.replace(pos,5,c_map);
		pos=file.find("%red%");
		if(pos!= std::string::npos) file.replace(pos,5,c_red);
		pos=file.find("%blu%");
		if(pos!= std::string::npos) file.replace(pos,5,c_blu);
		pos=file.find("%tag%");
		if(pos!= std::string::npos) file.replace(pos,5,c_tag);
